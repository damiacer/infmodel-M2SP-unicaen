...
# VALEURS DES PARAMETRES
  b<-para["b"]
  g<-para["g"]
# TRANSITIONS ENTRE COMPRATEMENTS : INTENSITES
  dS<--b*S*In/N
  dIn<-b*S*In/N-g*In
  dR<-g*In
  res <-c(dS,dIn,dR)
...
# PREDICTIONS THEORIQUES POUR LA TAILLE FINALE DE L'EPIDEMIE
computeSize<-function(N,R0,J)
{
  p<-function(x)
  {
    coeff<-c(1-R0,1/(2:J))
    return(sum(coeff*x^(0:(J-1))))
  }
...
# INITIALISATION
  ...
  delta_stop <- 0.0001
  res<-NA
  if (sign(p(inf0)) != sign(p(sup0))) 
  {
    inf <- inf0
    sup <- sup0
    delta_obs <- (sup - inf)
    while (delta_obs > delta_stop) 
    {
      milieu <- (sup + inf) / 2
      if (sign(p(inf)) == sign(p(milieu))) 
      {
        inf <- milieu
      }else{
        sup <- milieu
...
# DYNAMIQUE DE L'EPIDEMIE SELON LES DIFFERENTS VALEURS DE R0
R0varie<-function(R0,Duree,N,In0)
{
# VALEURS INITIALES
...
# MATRICE DE STOCKAGE DES RESULTATS
  valR<-valI<-matrix(nrow=length(times),ncol=length(R0))
  
  for (i in 1:length(R0))
  {
# VALEURS DE PARAMETRES
    param<-c(b=R0[i]/Duree,g=1/Duree)
...
# INTEGRATION DU SYSTEME
  ...
  par(mfrow=c(1,2))
  plot(times,valI[,1],col=1,ylim=range(0,max(valI)),main="Prevalence",type="l",xlab="Temps",ylab="Prevalence")
  for (i in 2:length(R0)) lines(times,valI[,i],col=i)
  legend("topright",paste("R0=",R0),col=1:length(R0),lty=1)
  
  plot(times,valR[,1],col=1,ylim=range(0,max(valR)),main="Recovered",type="l",xlab="Temps",ylab="Prevalence")
  for (i in 2:length(R0)) lines(times,valR[,i],col=i)
}
